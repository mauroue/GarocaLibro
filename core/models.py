...
# Classe Livro
class Livro(Base):
    STATUS_CHOICE = ((True, "Disponível"), (False, "Indisponível"))
...
class Emprestimo(Base):
    STATUS_CHOICE = (("in_progress", "Em andamento"), ("completed", "Finalizado"))

    devolucao = models.DateTimeField(
        "Data de devolução", help_text="Selecione uma data e hora posterior à atual"
    )
    status = models.CharField(
        "Status", max_length=20, choices=STATUS_CHOICE, default="in_progress"
    )
...
class Agendamento(Base):
    STATUS_CHOICE = (("scheduled", "Agendado"), ("completed", "Finalizado"))

    leitor = models.ForeignKey(Leitor, on_delete=models.CASCADE)
    livro = models.ForeignKey(Livro, on_delete=models.CASCADE)
    data_retirada = models.DateTimeField(
        "Data de Retirada", help_text="Escolha uma data e hora futuras"
    )
    status = models.CharField(
        "Status", max_length=20, choices=STATUS_CHOICE, default="scheduled"
    )
